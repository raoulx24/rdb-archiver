...
func Probe(dir string) Result {
	st, err := os.Stat(dir)
	if err != nil {
		return Result{false, true, fmt.Sprintf("stat failed: %v", err)}
	}
	if !st.IsDir() {
		return Result{false, true, "not a directory"}
	}

	w, err := fsnotify.NewWatcher()
	if err != nil {
		return Result{false, false, fmt.Sprintf("fsnotify unavailable: %v", err)}
	}
	defer w.Close()

	if err := w.Add(dir); err != nil {
		return Result{false, false, fmt.Sprintf("cannot watch directory: %v", err)}
	}
	...
	if f, err := os.Create(tmp); err == nil {
		...
		return Result{false, false, fmt.Sprintf("cannot create temp file: %v", err)}
	}

	// Rename temp - final to trigger a rename event.
	if err := os.Rename(tmp, final); err != nil {
		os.Remove(tmp)
		return Result{false, false, fmt.Sprintf("rename failed: %v", err)}
	}
	...
	for {
		...
		case <-timeout:
			return Result{false, false, "no events received (rename not reported)"}
		}
	}
}
